...
class PreferenceReasoningApp(ControlMixin, EventMixin, tk.Tk):
    ...
    def bind_events(self):
        ...
        self.reasoning_panel.result_tree.bind(
            "<ButtonRelease-1>", self.on_result_tree_select
        )
    ...
    def update_trace_display(self):
        ...
        for state, transition in current_trace:
            ...
            self.reasoning_panel.result_tree.insert(
                "", tk.END, values=(state_str, transition)
            )
    ...
    def load_demo_scenario1(self):
        ...
        self.spec_panel.state_space_entry.insert(
            0,
            "Color: {red, blue, green}; Size: {small, medium, large}; Shape: {circle, square}",
        )
        self.spec_panel.rules_tree.insert(
            "", tk.END, values=("1", "R1", "Color=red", "Size=medium")
        )
        self.spec_panel.rules_tree.insert(
            "", tk.END, values=("2", "R2", "Size=medium", "Color=green")
        )
        self.spec_panel.rules_tree.insert(
            "", tk.END, values=("3", "R3", "Color=green", "Size=large")
        )
        self.spec_panel.rules_tree.insert(
            "", tk.END, values=("4", "R4", "Shape=circle", "Color=red")
        )
        self.spec_panel.rules_tree.insert(
            "", tk.END, values=("5", "R5", "Color=blue", "Shape=square")
        )
        self.reasoning_panel.initial_state_entry.delete(0, tk.END)
        self.reasoning_panel.initial_state_entry.insert(
    ...
    def load_demo_scenario2(self):
        ...
        self.spec_panel.state_space_entry.insert(
            0,
            "Weather: {sunny, cloudy, rainy}; Temperature: {cold, mild, hot}; Day: {weekday, weekend}",
        )
        self.spec_panel.rules_tree.insert(
            "", tk.END, values=("1", "R1", "Weather=sunny", "Temperature=hot")
        )
        self.spec_panel.rules_tree.insert(
            "", tk.END, values=("2", "R2", "Day=weekday", "Weather=rainy")
        )
        self.spec_panel.rules_tree.insert(
            "", tk.END, values=("3", "R3", "Temperature=cold", "Weather=cloudy")
        )
        self.spec_panel.rules_tree.insert(
            "", tk.END, values=("4", "R4", "Weather=rainy", "Temperature=mild")
        )
        self.spec_panel.rules_tree.insert(
            "", tk.END, values=("5", "R5", "Temperature=mild", "Weather=sunny")
        )
        self.reasoning_panel.initial_state_entry.delete(0, tk.END)
        self.reasoning_panel.initial_state_entry.insert(
            0, "Weather=cloudy; Temperature=cold; Day=weekday"
        )
        self.reasoning_panel.target_state_entry.delete(0, tk.END)
    ...
    def load_demo_scenario3(self):
        ...
        self.spec_panel.state_space_entry.insert(
            0,
            "Speed: {slow, moderate, fast}; Fuel: {low, medium, high}; Engine: {off, on}",
        )
        self.spec_panel.rules_tree.insert(
            "", tk.END, values=("1", "R1", "Fuel=high", "Engine=on")
        )
        self.spec_panel.rules_tree.insert(
            "", tk.END, values=("2", "R2", "Engine=on", "Speed=moderate")
        )
        self.spec_panel.rules_tree.insert(
            "", tk.END, values=("3", "R3", "Speed=moderate", "Fuel=medium")
        )
        self.spec_panel.rules_tree.insert(
            "", tk.END, values=("4", "R4", "Fuel=medium", "Speed=fast")
        )
        self.spec_panel.rules_tree.insert(
            "", tk.END, values=("5", "R5", "Engine=off", "Fuel=high")
        )
        self.reasoning_panel.initial_state_entry.delete(0, tk.END)
        self.reasoning_panel.initial_state_entry.insert(
            0, "Speed=slow; Fuel=high; Engine=off"
        )
        self.reasoning_panel.target_state_entry.delete(0, tk.END)
