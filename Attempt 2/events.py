...
class EventMixin:
    ...
    def on_rule_tree_select(self, event):
        try:
            ...
            rule_id = (
                rule_tree.identify_row(event.y)
                if event
                else (rule_tree.selection()[0] if rule_tree.selection() else None)
            )
            if not rule_id:
                return
            ...
            for idx, state_id in enumerate(res_tree.get_children()):
                ...
                tag = (
                    "triggered"
                    if rule_triggered(selected_rule, state)
                    else "not_triggered"
                )
    ...
    def on_result_tree_select(self, event):
        try:
            res_tree = self.reasoning_panel.result_tree
            state_id = (
                res_tree.identify_row(event.y)
                if event
                else (res_tree.selection()[0] if res_tree.selection() else None)
            )
            if not state_id:
                return
            ...
            for rule_id in rule_tree.get_children():
                ...
                tag = (
                    "triggered"
                    if rule_triggered(rule, selected_state)
                    else "not_triggered"
                )
