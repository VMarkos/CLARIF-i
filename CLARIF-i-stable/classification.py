...
from classification_utils import (
    generate_classification_test_case,
...
def main():
    ...
    for n, i in tqdm(it.product(ns, range(n_its)), total=len(ns) * n_its):
        test_case = generate_classification_test_case(
            n, find_classification_inertia_action, keep_advice_track=True
        )
        test_case.run()
        results[f"{n}-{i}"] = {
            "report": test_case.report(),
            "advice_track": [
                ([str(f[0]) for f in p], a[-1].name) for p, a in test_case.advice_track
            ],
        }
