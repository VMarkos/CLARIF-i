...
from utils import compute_fidelity, find_bubble_swap_action, find_bubble_partial_swap_action, find_quick_swap_action, find_quick_partial_swap_action
...
CONFIGS = it.product( (True, False), (True, False), COACH_ACTIONS.keys() )

CONFIG_COLORS = dict(zip(COACH_ACTIONS.keys(), zip(COLORS, MARKERS)))

STYLE_CONFIGS_MAP = lambda c: (*CONFIG_COLORS[c[2]], 'dotted' if c[1] else 'dashed' if c[0] else 'solid')
...
def compute_fidelities(traces: list) -> list:
    """ traces as parsed by `parse_traces()` """
    fidelities = dict()
    for ts, config in zip(traces, CONFIGS):
        if not config[0] and config[1]:
            continue
        coach_action_fn = COACH_ACTIONS[config[-1]]
        fidelities[config] = { n: [ compute_fidelity(t, coach_action_fn) for t in trs ] for n, trs in ts.items() }
    return fidelities

def plot_fidelities(fidelities: dict, save_path)-> None:
    config_to_str = lambda c: f"{c[2]}, Mem: {'Long' if c[1] else 'Short' if c[0] else 'None'}"
    fig, ax = plt.subplots()
    handles = []
    for config, fidelity in fidelities.items():
        ns, mean_fs, std_fs = zip(*( (n, mean(fs), stdev(fs)) for n, fs in fidelity.items() ))
        ...
        h, = ax.plot(ns, mean_fs, label=config_to_str(config), color=c, linestyle=ls, marker=m, alpha=0.9)
        handles.append(h)
    ax.grid()
    ax.legend(handles=sorted(handles, key=lambda h: h._label)) # TODO add markers and sort legend
    fig.tight_layout()
...
def main():
    ...
    N, r, s = vars(parser.parse_args()).values()
    ...
    print('Magic happens...')
    fidelities = compute_fidelities(traces)
    print('Creating plot...')
