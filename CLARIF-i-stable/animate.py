...
class SortingAnimator:
    def __init__(self, traces_path: str, interval: int = 400) -> None:
        ...
        self.traces: dict[tuple[int, int], list[list[int]]] = self.__parse_traces(
            traces_path
        )
        ...
        with open(path, "r") as file:
            ...
            for line in file:
                ...
                if is_header:
                    ...
                    trace = [
                        [int(w.split("=")[-1]) for w in word.split(",")]
                        for word in words
                    ]
                    trace = [t for i, t in enumerate(trace) if t not in trace[:i]]
                    traces[current_key] = (
                        trace  # NOTE This skips intentionally everything except for the last iteration; a more efficient way must exist
                    )
        return traces
    ...
    def generate(self, key: tuple[int, int]) -> None:
        ...
        fig, ax = plt.subplots()
        plt.suptitle(
            f"{self._algorithm}: N={self._n}, iteration {key[1] + 1} / {self._reps}",
            y=0.99,
            fontsize=18,
        )
        plt.title(
            f"With{'' if self._mem else 'out'} Memory; {'' if self._long else 'Not'} Long",
            fontsize=10,
        )
        ...
        differences = (
            ...
            for x, y in zip(traces[:-1], traces[1:])
        )
        ...
        self.anim = animation.ArtistAnimation(
            fig=fig, artists=artists, interval=self.interval
        )
...
def sorting_animator(input_path: str):
    ...
    save_path = os.path.join(
        ANIMATIONS_PATH, input_path[:-6] + "_" + "_".join(map(str, key)) + ".gif"
    )
