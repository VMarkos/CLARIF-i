...
def main():
    ...
    trace_file_name = os.path.join(RESULTS_PATH, f"{algorithm}_{coach_type}_test_N{N}_reps{reps}_mem{memory}_long{long_memory}.trace")
    FORMAT = "%(asctime)s :: %(message)s"
    log_file_name = os.path.join(LOGS_PATH, f"{algorithm}_{coach_type}_test_N{N}_reps{reps}_mem{memory}_long{long_memory}.log")
    ...
    for n in range(1, N + 1):
        ...
        for i in tqdm(range(reps)):
            test = ALGORITHMS[algorithm](n, N, learner, coach_class, full_reporting, report_traces, randomize_targets)
            try:
                test.run()
