...
class CoachedSwapSortingEnv(gym.Env):
  """Coached sorting env restricted to adjacent (bubble-style) swaps via action masking."""
  ...
  def action_masks(self):
    """Allow only adjacent swaps (i, i+1) within the active prefix [0, current_n)."""
    mask = np.zeros(self.num_actions, dtype=bool)
    for idx, (i, j) in enumerate(self.swap_pairs):
      if j == i + 1 and j < self.current_n:
        mask[idx] = True
    return mask
...
def plot_coaching_results(results):
  ...
  fig.suptitle(
      "Coached RL Agent (Adjacent Swaps Only - SubprocVecEnv)",
      fontsize=14,
      fontweight="bold",
  )
  fig.tight_layout()
...
if __name__ == "__main__":
  ...
  curriculum_callback = CurriculumCallback(
      eval_env=eval_env,
      target_success_rate=0.85,
      eval_freq=1000,  # Evaluates every 1000 calls
      n_eval_episodes=20,
  )
  ...
  print(
      f"Starting Bubble-Masked Coached RL Training with SubprocVecEnv"
      f" ({NUM_ENVS} workers)..."
  )
  model.learn(total_timesteps=1_000_000, callback=curriculum_callback)
  print("Training Complete!\n")
  model.save('final_lev_bubble.zip')

  # Run evaluation routine
  print("Evaluating Coached Agent (Adjacent Swaps)...")
