...
class CoachedSwapSortingEnv(gym.Env):
    ...
    for i in range(n):
      for j in range(0, n - i - 1):
        if arr[j] > arr[j + 1]:
          ...
          # Map adjacent swap (j, j+1) to discrete action ID
          pair = (j, j + 1)
  ...
  def reset(self, seed=None, options=None):
    ...
    # Generate coach's optimal trace for this initial layout
    self.coach_trace = self._generate_bubble_sort_trace(
        self.state[: self.current_n]
    )

    # Initial Levenshtein distance between empty agent trace and coach trace
  ...
  def step(self, action):
    ...
    # Calculate current trace distance to coach
    current_distance = levenshtein_distance(self.agent_trace, self.coach_trace)

    # Reward shaping based on Levenshtein progress
    ...
    delta_inv = self.prev_inversions - inversions
    self.prev_inversions = inversions

    reward += float(delta_inv) * 2.0

    terminated = inversions == 0

    if terminated:
      reward += 20.0  # Goal completion bonus
  ...
  def _on_step(self) -> bool:
    if self.n_calls % self.eval_freq == 0:
      ...
      if (
        ...
        self.current_n += 1
        self.n_eval_episodes = max(100, 2 * self.current_n ** 2)
        if self.verbose > 0:
...
# ==========================================
# 4. Evaluation Routine & Fidelity Metrics
# ==========================================


def evaluate_coached_agent(model, env, eval_stages, num_episodes=50):
  """Evaluates agent sorting success rate and coach fidelity across curriculum stages."""
  results = {}
  ...
  for n in eval_stages:
    ...
    for _ in range(num_episodes):
      ...
      # Compute normalized Levenshtein similarity (Fidelity)
...
def plot_coaching_results(results):
  """Plots Success Rate, Coach Fidelity, and Avg Steps across n."""
  ...
  fig.suptitle(
      "Coached RL Agent (Bubble Sort Trace Alignment)",
      fontsize=14,
      fontweight="bold",
  )
  fig.tight_layout()
...
if __name__ == "__main__":
  ...
  train_env = CoachedSwapSortingEnv(
  ...
  print("Starting Coached RL Training...")
  model.learn(total_timesteps=100_000, callback=curriculum_callback)
  print("Training Complete!\n")
  model.save(f'lev_{MAX_N}_10000000.zip')

  print("Evaluating Coached Agent...")
  eval_stages = list(range(3, MAX_N + 1))
  test_results = evaluate_coached_agent(
      model, eval_env, eval_stages=eval_stages, num_episodes=100
  )
