...
class ScenarioManager:
    ...
    def load_scenario(self, scenario_name: str) -> Dict[str, Any]:
        ...
        data["feedback_rules"] = [
            Rule.from_dict(rule) for rule in data["feedback_rules"]
        ]
    ...
    def save_scenario(self, scenario_name: str, data: Dict[str, Any]) -> None:
        ...
        data_to_save["learner_rules"] = [
            rule.to_dict() for rule in data["learner_rules"]
        ]
        data_to_save["coach_rules"] = [rule.to_dict() for rule in data["coach_rules"]]
        data_to_save["feedback_rules"] = [
            rule.to_dict() for rule in data["feedback_rules"]
        ]
...
DEMO_SCENARIOS = {
    ...
    "traffic_light": {
        ...
        "coach_rules": [
            Rule(
                "Warn about red",
                {"light": "red", "speed": "normal"},
                {"action": "warn_slow"},
                1,
            ),
            Rule(
                "Remind to stop",
                {"position": "near", "speed": "fast"},
                {"action": "warn_stop"},
                1,
            ),
        ],
